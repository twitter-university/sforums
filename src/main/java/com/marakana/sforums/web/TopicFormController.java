...
public class TopicFormController {
    ...
    Long id, @RequestParam(value = "forumId", required = false)
    Long forumId) {
        Topic topic;
        if (id == null) {
            topic = new Topic();
    ...
    public String processSubmit(@ModelAttribute("topic")
    @Valid
    Topic topic, BindingResult result, SessionStatus status) {
        if (!result.hasErrors()) {
            try {
